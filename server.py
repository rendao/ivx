...
DEFAULT_PROJECT_NAME = os.getenv("DASHBOARD_DEFAULT_PROJECT", "generic-project")
...
def project_id_for(name: str, project_path: str) -> str:
    ...
    if project_path:
        return f"{base}-{short_hash(str(Path(project_path).expanduser().resolve()))}"
    return base
...
def public_project_meta(meta: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    return {
        "id": str(source.get("id") or "default"),
        "name": str(source.get("name") or DEFAULT_PROJECT_NAME),
        "path": mask_project_path(raw_path),
...
def save_registry(registry: Dict[str, Any]) -> None:
    ...
    PROJECT_STATE_FILE.write_text(json.dumps(registry, ensure_ascii=False, indent=2), encoding="utf-8")
...
def build_bootstrap_progress(project_meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    planned = max(8, min(120, max(1, files) // 6 if files else 10))
    done = min(planned, max(0, py_files // 12 + md_files // 20))
...
def apply_git_activity(payload: Dict[str, Any], project_meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    project_path = str(project_meta.get("path") or "")
    activity = collect_git_activity(project_path)
    test_activity = collect_test_activity(project_path)
    ...
    # For auto-constructed contracts, replace placeholder phase/task/progress
    # with git-derived values so the board reflects real project movement.
...
def attach_project_meta(progress: Dict[str, Any], project_meta: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    progress["project_id"] = str(meta.get("id") or "default")
    progress["project"] = str(meta.get("name") or progress.get("project") or DEFAULT_PROJECT_NAME)
    progress["project_path"] = mask_project_path(raw_path)
...
def read_progress() -> Dict[str, Any]:
    with FILE_LOCK:
        if not PROGRESS_FILE.exists():
            ...
            return data
        data = attach_project_meta(json.loads(PROGRESS_FILE.read_text(encoding="utf-8")))
        debug_log(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run live progress dashboard server (service process only; use integrate_project.py for integration orchestration)"
    )
    ...
    if not PROGRESS_FILE.exists():
        PROGRESS_FILE.write_text(json.dumps(default_progress(current if isinstance(current, dict) else None), ensure_ascii=False, indent=2), encoding="utf-8")
