...
def _run_integration(args: list[str]) -> int:
    ...
    server_script = ROOT / "app.py"
    push_script = ROOT / "push_live_progress.py"
...
def main() -> int:
    ...
    if command == "serve":
        return _run_script("app.py", rest)
    if command in {"integrate", "i"}:
        return _run_integration(rest)
    if command == "push-progress":
        return _run_script("push_live_progress.py", rest)
    if command == "push-collaborator":
        return _run_script("push_collaborator_update.py", rest)
    if command == "push-ci":
        return _run_script("push_ci_result.py", rest)
